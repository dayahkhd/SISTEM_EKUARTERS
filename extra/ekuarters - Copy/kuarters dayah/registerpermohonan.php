...
$datajawatan	= mysql_fetch_array($resultjabatan);

$jawatandb = $datajawatan['jabatan'];
...
if (! (isset ( $_SESSION ['nokadpengenalan'] ))) {
	...
	if(isset($_POST['submit']) == "Simpan"){
	{
		...
		if ($katjawatan == 1) {
			...
			$query2 = "SELECT * FROM tblkeluarga WHERE k_id = '$keluarga'" ;
			...
			$query7 = "SELECT * FROM tblkeluarga WHERE k_id = '$keluarga'";
		...
		(p_tarikhmohon, p_lokasi, p_idstaff, p_jantina, p_statusperkahwinan, p_telrumah, p_telhp, p_telpejabat,p_katjawatan, p_jawatan, p_jabatan, p_unit, p_tmulabertugas, p_waktubertugas,  p_namapasangan, p_icpasangan, p_jawatanpasangan, p_tkerjapasangan, p_wbpasangan, p_bilanak, p_alamatsekarang, p_checkbox1, p_checkbox2, p_checkbox3, p_alamatrumahsd, p_jarakrumah, p_sebabmohon, p_pengesahan, p_totalmarkah)
		VALUES
		('$tmohon', '$lokasimohon', '$idstaff', '$jantina', '$statusperkahwinan', '$telrumah', '$telhp', '$telpejabat','$katjawatan','$jawatan', '$tbertugas', '$unit', '$tberkhidmat', '$officehour', '$namapasangan', '$nokppasangan', '$jawatanpasangan', '$tbpasangan','$wbpasangan', '$bilanak', '$alamatlama', '$checkbox1', '$checkbox2', '$checkbox3', '$alamatsendiri', '$jarakrumah', '$sebabmohon', '$pengesahan', '$xtotalmarkah')";
...
<form method="post" >
	...
	<div id="page-wrapper">
	    ...
	    ?> -->
			<div class="row">
				<div class="col-lg-12">
					<h4 class="page-header"> SELAMAT DATANG <strong><em><?php echo  $nama;?></strong></em></h4>
				</div>
			</div>
			      ...
			      <div class="col-lg-6">
			        <label for="tbertugas">:</label>
						<?php
							...
							echo "<option value='$tbertugas'>$jawatandb</option>";

							while($_POST = mysql_fetch_array($result1))
							{
			...
			<div class="row">
			  <div class="col-lg-12">
			    ...
			    <div class="form-group">
			      ...
			      <div class="col-lg-6">
			        <p>
			          <label for="kkeluarga">:</label>
                      <div name="kkeluarga" id="kkeluarga">
						   <?php
									$query_kk = "select * from tblkeluarga";
									$result_kk = mysql_query($query_kk) or die (mysql_error());

									echo "<select class=form-control name=kkeluarga id=kkeluarga >";
									echo "<option value = ->Sila Pilih...</option>";

									while($data_kk = mysql_fetch_array($result_kk))
									{
									echo "<option value = $data_kk[k_id]>$data_kk[k_nama]</option>";
									}
									echo "</select>";		   
						?>
                      </div>
