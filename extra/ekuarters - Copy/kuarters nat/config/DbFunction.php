...
class DbFunction{
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM tablepengguna where nokadpengenalan=? and katalaluan=? ";
...
function showStudents1($id){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM tablepengguna where nokadpengenalan='".$id."'";
	$stmt= $mysqli->query($query);
	return $stmt;
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
		           ...
		           //	echo $session;exit;
   $query = "UPDATE INTO `tablepengguna` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
